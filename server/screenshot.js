...
const targetURL = argv.url || 'http://docker.i.louie.se:8123/lovelace-kobo/weather';
const targetFile = argv.filename || 'desktop.png'
const viewport = [600,1200];
...
CDP(async function(client){
  ...
  setTimeout(async function() {
    ...
    fs.writeFile(targetFile, buffer, 'base64', function(err) {
      if (err) {
        console.error(err);
      } else {
